      ...
      <form id="myForm" action="{{ route('update.blog.post') }}" method="post" class="row g-3"
        ...
        <div class="form-group col-md-6">
          ...
          <select name="blogcat_id" class="form-select mb-3" aria-label="Default select example">
            ...
            <option value="{{ $cat->id }}" {{ $cat->id == $post->blogcat_id ? 'selected' : '' }} >{{ $cat->category_name
              }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-md-6">
          <img id="showImage" src="{{ asset($post->post_image) }}" alt="Admin" class="rounded-circle p-1 bg-primary"
            width="80">
        </div>
